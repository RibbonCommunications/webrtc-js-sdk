...
The SIP URI ie: sip:[joe@domain.com][26]
...
*   `deviceId` **[call.MediaConstraint][35]** ID of the microphone to receive audio from.
...
*   `deviceId` **[call.MediaConstraint][35]** ID of the camera to receive video from.
*   `height` **[call.MediaConstraint][35]?** The height of the video.
*   `width` **[call.MediaConstraint][35]?** The width of the video.
*   `frameRate` **[call.MediaConstraint][35]?** The frame rate of the video.
...
*   `height` **[call.MediaConstraint][35]?** The height of the screen capture.
*   `width` **[call.MediaConstraint][35]?** The width of the screen capture.
*   `frameRate` **[call.MediaConstraint][35]?** The frame rate of the live screen share.
...
*   `urls` **([Array][20]<[string][8]> | [string][8])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][38] to learn more about the actual url format.
...
*   `tracks` **[Array][20]<[call.TrackObject][44]>** A list of Track objects that are contained in this Media object.
...
*   `camera` **[Array][20]<[call.DeviceInfo][45]>** A list of camera device information.
*   `microphone` **[Array][20]<[call.DeviceInfo][45]>** A list of microphone device information.
*   `speaker` **[Array][20]<[call.DeviceInfo][45]>** A list of speaker device information.
...
*   `media` **[Object][7]** The media options the call should be initialized with.
    ...
    *   `media.audioOptions` **[call.AudioOptions][59]?** Options for configuring the call's audio.
    *   `media.video` **[boolean][11]** Whether the call should have video on start. (optional, default `false`)
    *   `media.videoOptions` **[call.VideoOptions][60]?** Options for configuring the call's video.
    ...
    *   `media.screenOptions` **[call.ScreenOptions][61]?** Options for configuring the call's screenShare.
...
*   `options` **[Object][7]?** 
    ...
    *   `options.customParameters` **[Array][20]<[call.CustomParameter][34]>?** Custom SIP header parameters for the SIP backend.
...
will be notified, through their own [call:stateChange][63]
...
[call:trackEnded][65] event for more
information.
...
The SDK will emit a [call:trackEnded][65]
...
The SDK will emit a [call:tracksAdded][68]
...
*   `callId` **[string][8]** ID of the call being acted on.
*   `videoOptions` **[call.VideoOptions][60]?** Options for configuring the call's video.
*   `options` **[Object][7]?** 
...
The SDK will emit a [call:tracksRemoved][70]
event for both the local and remote users to indicate that a track
has been removed.
...
*   `media` **[Object][7]** The media options. (optional, default `{}`)
    ...
    *   `media.audioOptions` **[call.AudioOptions][59]?** Options for configuring the audio track.
    *   `media.video` **[boolean][11]** Whether to create a video track. (optional, default `false`)
    *   `media.videoOptions` **[call.VideoOptions][60]?** Options for configuring the video track.
...
the Call, such as local video or remote audio, can be [muted][74]
or [unrendered][75] during this operation if desired.

This operation will use the browser's
[Audio constructor][76]
...
The SDK will emit [call:operation][54] events locally
...
[media:trackEnded][77] event will be emitted
...
the [call.replaceTrack][78] API to revert the local audio track.
...
Returns **[Promise][79]** Promise that resolves when the operation is complete.
...
Returns **[Array][20]<[call.CallObject][80]>** Call objects.
...
Returns **[call.CallObject][80]** A call object.
...
Set the [Custom Parameters][81] of a Call, to be provided to the remote endpoint.
...
*   `callId` **[string][8]** The ID of the call.
*   `customParameters` **[Array][20]<[call.CustomParameter][34]>** The custom parameters to set.
...
To set, change or remove the custom parameters on a call, use the [call.setCustomParameters][48] API.
...
The data retrieved is a [RTCStatsReport][82]
...
The SDK will emit a
[call:statsReceived][84] event, after
...
Returns **[Promise][79]** A promise that will resolve with the stats report or an error if it fails.
...
In addition, the SDK emits a [call:availableCodecs][85] event
upon retrieving that list of codecs.

This API is a wrapper for the static method [RTCRtpSender.getCapabilities()][86].
...
Returns **[Promise][79]** A promise that will resolve with an object containing the available codecs, along with the `kind` parameter, that was supplied in the first place.
...
See event documentation [here][87].
See metrics documentation [here][88].
...
Set [SDP Handler Functions][89] that will be run as part of a pipeline for all future calls.
...
problem. Using the [call.restartMedia][91] API will attempt to reconnect the media. See
the [call.restartMedia][91] API description for more information.

These states are direct reflections of the possible
[RTCPeerConnection.iceConnectionState][92] values.
...
*   `FAILED` **[string][8]** The connection has failed and cannot be recovered automatically. A full media connection refresh is required to reestablish a connection. See the [call.restartMedia][91] API.
...
The call report can be retrieved via the [call.getReport][93] API.
...
[CallObject][51]. This event indicates that an operation's
...
[call:stateChange][63] event should be used for this purpose.
...
to the Call, using the [call.addMedia][67] API for example, or when the
...
Information about a Track can be retrieved using the [media.getTrackById][94] API.
...
tracks, by using the [call.removeMedia][69] API for example, or when the
Call is held with the [call.hold][95] API.
...
See the [call.getStats][96] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][97]?** The RTCStatsReport.
    *   `params.error` **[api.BasicError][25]?** An error object, if the operation was not successful.
...
A local Track has been replaced by the [call.replaceTrack][78] API.
...
available, similar to the [call:tracksRemoved][70]
event, and the new Track is available in its place, similar to the
[call:tracksAdded][68] event. The event
...
[CustomParameter][81] for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.customParameters` **[Array][20]<[call.CustomParameter][34]>** The custom parameters received.

### call:availableCodecs

The list of available and supported codecs by the browser have been retrieved.

This event is emitted as a result of the [call.getAvailableCodecs][98] API. Please refer to the API for more
information.
...
This event is emitted as a result of the [call.restartMedia][91] API being called.
See the description for [call.restartMedia][91] for information about its
usage.
...
[CallHistory.get][99] can then be used to get
...
[call.history.setCache][100] API. This allows an
...
The data can be retrieved from the [call.history.getCache][101] API. This allows an
application to persist the information across SDK instances when the
backend environment does not support the CallHistory feature.
...
Call history state has been updated. See [CallHistory.get][99] to retrieve new state.
...
Can be retrieved using the [connection.getSocketState][102] API.
...
Returns **[connection.WSConnectionObject][103]** Details about the current websocket connection, including state and configuration.
...
notably the [ws:change][104] event.
...
Returns **[conversation.Conversation][105]** A Conversation object matching the passed destination, otherwise undefined is returned.

### getAll

Returns all conversations currently tracked by the SDK

Returns **[Array][20]<[conversation.Conversation][105]>** An array of conversation objects.
...
Returns **[conversation.Message][107]** The newly created Message object.
...
A message object can be obtained through the [Conversation.createMessage][106] API on an existing conversation.

Messages have Parts which represent pieces of a message. Currently, only a 'text' part is suported.
Once all the desired parts have been added to the message using the [Message.addPart][108] function,
the message can then be sent using the [Message.send][109] function.
...
obtained through the [Conversation.getMessages][110] or [Conversation.getMessage][111] API on an existing conversation.
...
*   `type` **[string][8]** The type of message that was sent. See [conversation.chatTypes][112] for valid types.
...
[config.logs][113]) to customize how the logs are
...
[config.logs][113]) will need to handle LogEntry
objects.
...
[config.logs][113]). The SDK will then provide this
...
SDK has access to. Media is used in conjunction with the [Calls][114]
...
the [media.getDevices][115] API.

### getDevices

Retrieves the available media devices for use.

The [devices:change][116] event will be
...
Returns **[call.MediaObject][117]** A Media object.
...
For device information, the [media.getDevices][115] API will retrieve
...
recommend using the [media.initializeDevices][118] API in this
...
The SDK will emit a [devices:change][116]
event when the operation is successful or a
[devices:error][119] event if an error is
encountered.
...
Returns **[Promise][79]** Resolves with an array of objects containing both a media type and a media object.
...
Returns **[Promise][79]<[undefined][90]>** 
...
The SDK will emit a [media:muted][120] event
...
The SDK will emit a [media:unmuted][121] event
...
[MediaConstraint(s)][122] that was provided.

#### Parameters

*   `params` **[Object][7]** 

    *   `params.error` **[Error][123]** The Basic error object.

### media:muted

The specified Tracks have been muted.

A Track can be muted using the [media.muteTracks][74] API.
...
A Track can be unmuted using the [media.unmuteTracks][124]
API.
...
[media:sourceMuted][126] event), and will
...
[call.removeMedia][69] API so the remote side of the Call knows the track
has stopped, or the track can be replaced with a new track using the
[call.replaceTrack][78] API to prevent any interruption.
...
available either by using [presence.fetch][127] or by subscribing for
updates about other Users, using [presence.subscribe][128].

Available presence information can be retrieved using [presence.get][129] or
[presence.getAll][130].
...
An instance of this type can be obtained by invoking the [presence.get][129] function.
...
The status is set to [open][131] as soon as a user subscribes for the presence service.
...
*   `status` **[string][8]** The current status the user has set for themselves. For supported values see [presence.statuses][131].
*   `activity` **[string][8]** The current activity of the user.
    For supported values see [presence.activities][132].
...
See [presence.statuses][131] and [presence.activities][132] for valid
values.

The SDK will emit a
[presence:selfChange][133] event
when the operation completes. The updated presence information is
available and can be retrieved with [presence.getSelf][134].

Other users subscribed for this user's presence will receive a
[presence:change][135] event.
...
[presence.get][129] or [presence.getAll][130] APIs.
...
This information is set using the [presence.update][136] API.
...
This event is generated as a result of [presence.fetch][127] or [presence.update][136] operations.
...
The changed information can be retrieved using the [presence.get][129]
API.
...
The changed information can be retrieved using the [presence.getSelf][134]
API.
...
On completion, this API will trigger a [proxy:change][137]
event on success. The [proxy.getProxyMode][138] or [proxy.getInfo][139]
APIs can be used to verify that proxy mode has been changed. If an error
is encountered, this API will trigger a [proxy:error][140]
event.
...
When set to `true`, WebRTC operations will be proxied over a channel. When
set to `false`, WebRTC operation will occur as normal on the local machine.
See the [proxy.setProxyMode][141] API for more information.
...
On completion, this API will trigger a [proxy:change][137]
event on success. The [proxy.getInfo][139] API can be used to verify
that a channel has been set. If an error is encountered, this API will
trigger a [proxy:error][140] event. A channel
cannot be set if there is an on-going call.

#### Parameters

*   `channel` **[proxy.Channel][142]** See the `Channel` module for information.
...
(see the [API][143]). It is recommended to perform this
step before setting the proxy mode (see the [proxy.setProxyMode][141] API).

On completion, this API will trigger a [proxy:change][137]
event on success. The [proxy.getInfo][139] API can be used to verify
that the remote endpoint is initialized. If an error is encountered, this
API will trigger a [proxy:error][140] event.
...
The [proxy.getInfo][139] API can be used to retrieve the current proxy state.
...
This is a wrapper on top of the browser's [fetch API][144]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][144]
...
Returns **[Promise][79]<[Response][145]>** A promise for a [Response][146] object.

## sdpHandlers

A set of [SdpHandlerFunction][89]s for manipulating SDP information.
...
[call.setSdpHandlers][147] function.
...
using the [services.getSubscriptions][148] API. These values can be used
with the [services.subscribe][149] API.
...
the [services.subscribe][149] function.
...
This is the configuration object that needs to be passed as the value for the [ServiceDescriptor.params][150] property.
...
For push notifications on WebRTC Gateway, please see [notifications.registerPush][151]

The SDK currently only supports the `websocket` channel as a subscription
type.

When calling this API, SDK emits a [subscription:change][152] event, each time there is a change in subscriptions.

Upon getting such event, existing subscriptions can be retrieved using the
[services.getSubscriptions][148] API.

#### Parameters

*   `services` **[Array][20]<([string][8] | [services.ServiceDescriptor][153])>** A list of service configurations.
...
Upon getting such event, existing subscriptions can be retrieved using the
[services.getSubscriptions][148] API. The `subscribed` values are the
...
To be notified when any subscription(s) did change/complete, listen for [subscription:change][152] events.
...
These values are used in the [subscription:change][152] event
...
The value for this parameter will be one of the [services.changeReasons][154] constants.
...
[sip.subscribe API][155] for more information about solicited events.
...
provisioned as part of the user subscription using the [services.subscribe][149]
API.
...
from a subscription using the [sip.update][156] API at any time.

The SDK will emit a [sip:subscriptionChange][157]
event when the operations completes. The [sip.getDetails][158] API can be used
to retrieve the current information about a subscription.

The SDK will emit a [sip:eventsChange][159] event when
...
*   `customParameters` **[Array][20]<[call.CustomParameter][34]>?** Custom SIP header parameters for the SIP backend.
...
Subscription details will no longer be available using the [sip.getDetails][158]
...
This event can be emitted when a new SIP subscription is created ([sip.subscribe][155]
API), an existing subscription is updated ([sip.update][156] API), or has been
deleted ([sip.unsubscribe][160] API). The `change` parameter on the event indicates
...
The [sip.getDetails][158] API can be used to retrieve the current information about
a subscription.
...
The ID of a User (e.g. [joe@domain.com][26])
...
The SDK will emit a [users:change][161]
...
See the [user.fetch][163] and [user.search][164] APIs for details about
...
Returns **[user.User][165]** The User object for the specified user.
...
Returns **[Array][20]<[user.User][165]>** An array of all the User objects.
...
The SDK will emit a [directory:change][166]
...
[user.get][162] and [user.getAll][167] APIs.
...
*   `params` **[Object][7]** 
    ...
    *   `params.results` **[Array][20]<[user.User][165]>** The Users' information returned by the
        operation.
...
A [voicemail:change][168] event is
...
[65]: call.event:call:trackEnded

[66]: #callunhold

[67]: #calladdmedia

[68]: #calleventcalltracksadded

[69]: #callremovemedia

[70]: #calleventcalltracksremoved

[71]: #calleventcalltrackreplaced
...
[76]: https://developer.mozilla.org/en-US/docs/Web/API/HTMLAudioElement/Audio

[77]: #mediaeventmediatrackended
...
[81]: #callcustomparameter

[82]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStatsReport

[83]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStats

[84]: #calleventcallstatsreceived

[85]: #calleventcallavailablecodecs

[86]: https://w3c.github.io/webrtc-pc/#dom-rtcrtpsender-getcapabilities

[87]: #callreportevents

[88]: #callmetrics

[89]: #callsdphandlerfunction

[90]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/undefined

[91]: #callrestartmedia

[92]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceConnectionState

[93]: #callgetreport

[94]: #mediagettrackbyid
...
[100]: callHistory.setCache

[101]: callHistory.getCache

[102]: #connectiongetsocketstate

[103]: #connectionwsconnectionobject

[104]: #connectioneventwschange

[105]: #conversationconversation

[106]: #conversationconversationcreatemessage

[107]: #conversationmessage

[108]: conversation.Message.addPart

[109]: #conversationmessagesend

[110]: #conversationconversationgetmessages

[111]: #conversationconversationgetmessage

[112]: conversation.chatTypes

[113]: #configconfiglogs

[114]: #call

[115]: #mediagetdevices

[116]: #mediaeventdeviceschange

[117]: #callmediaobject

[118]: #mediainitializedevices

[119]: #mediaeventdeviceserror

[120]: #mediaeventmediamuted

[121]: #mediaeventmediaunmuted

[122]: #callmediaconstraint

[123]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error

[124]: #mediaunmutetracks

[125]: #mediaeventmediasourceunmuted

[126]: #mediaeventmediasourcemuted

[127]: #presencefetch

[128]: #presencesubscribe

[129]: #presenceget

[130]: #presencegetall

[131]: #presencestatuses

[132]: #presenceactivities

[133]: #presenceeventpresenceselfchange

[134]: #presencegetself

[135]: #presenceeventpresencechange

[136]: #presenceupdate

[137]: #proxyeventproxychange

[138]: #proxygetproxymode

[139]: #proxygetinfo

[140]: #proxyeventproxyerror

[141]: #proxysetproxymode

[142]: #proxychannel

[143]: #proxysetchannel

[144]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[145]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5

[146]: https://developer.mozilla.org/en-US/docs/Web/API/Response

[147]: #callsetsdphandlers

[148]: #servicesgetsubscriptions

[149]: #servicessubscribe

[150]: #servicesservicedescriptor

[151]: notifications.registerPush

[152]: #serviceseventsubscriptionchange

[153]: #servicesservicedescriptor

[154]: #serviceschangereasons

[155]: #sipsubscribe

[156]: #sipupdate

[157]: #sipeventsipsubscriptionchange

[158]: #sipgetdetails

[159]: #sipeventsipeventschange

[160]: #sipunsubscribe

[161]: #usereventuserschange
...
[166]: #usereventdirectorychange

[167]: #usergetall

[168]: #voicemaileventvoicemailchange
